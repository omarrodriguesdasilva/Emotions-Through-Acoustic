...
def feature_and_train(samples_prefix, st_w, st_s, mt_w, mt_s, perc_train, confusion_matrix_perc, use_svm, 
    ...
    if use_svm:
        ...
        best_perf = aT.extract_features_and_train(list_of_dirs_or_classes, mt_w, mt_s, st_w, st_s,
                                                  "svm", model_name, False, perc_train,
                                                  None, False)
        end = time.time()
        ...
        print_parameters(st_w, st_s, mt_w, mt_s, perc_train, use_svm, model_name)
        best_perf = aT.extract_features_and_train(list_of_dirs_or_classes, mt_w, mt_s, st_w, st_s,
                                                  "knn", model_name, False, perc_train,
                                                  None, False)
        end = time.time()
        print(" Finished in {:10.2f} seconds ".format(end - start))
        # Alterado Omar - 17/12/2022
    ...
    if True is True:
        ...
        if verbose:
            ...
            print("\n \n The audio file was classified as {} with prob {}% in {:.2f} seconds \n \n".
                  format(chosenClass.upper(), round(chosen * 100, 2), end - start))
        return chosenClass, chosen, classNames
    else:
...
def main_SVM(filename, fold):
    ...
    if filename != 'TRTE-TRAIN':
        test_model(TEST_PREFIX, MODEL_NAME, fold, filename)

def main_SVM_serieem(direm, filename, file_to_test, fold):
    print("__name__: ", __name__)
    PROJECT_PATH = os.path.dirname(os.path.abspath(__file__))
    print("project_path:", PROJECT_PATH)
    print("os.listdir:", os.listdir(PROJECT_PATH))
    print("os.path.dirname:", os.path.dirname(PROJECT_PATH))
    print("Teste arq-serie emocional")
    TEST_PREFIX = PROJECT_PATH + direm
    MODEL_NAME = filename + str(fold)
    print("model_name=", MODEL_NAME)
    use_svm = True
    MODEL_NAME = "models/SVM_ " + MODEL_NAME
    return test_file(file_to_test, MODEL_NAME, use_svm)