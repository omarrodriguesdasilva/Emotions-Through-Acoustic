...
def multiple_directory_feature_extraction(path_list, mid_window, mid_step,
    ...
    class_names = []
    file_names = []
    # incluido omar para extração de características - 09/12/2023
    feature_names_arr = []
    # fim incluido omar para extração de características - 09/12/2023
    for i, d in enumerate(path_list):
        ...
        if f.shape[0] > 0:
            ...
            # Alterado em 31/12/2023 devido à execução sob windows.
            # O os.sep retorna "\" e gera o class_names com o diretório inteiro ao invés do nome das classes.
            # if d[-1] == os.sep:
            if d[-1] == '/':
                class_names.append(d.split('/')[-2])
            else:
                class_names.append(d.split('/')[-1])
            # incluido omar para extração de características - 09/12/2023
            feature_names_arr.append(feature_names)
            # fim incluido omar para extração de características - 09/12/2023
    return features, class_names, file_names, feature_names_arr
